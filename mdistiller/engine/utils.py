...
def adjust_learning_rate(epoch, cfg, optimizer):
    if hasattr(cfg.SOLVER, 'LR_COS') and cfg.SOLVER.LR_COS:
        # Cosine annealing learning rate scheduler
        print('cos')
        warmup_epochs = getattr(cfg.SOLVER, 'WARMUP_EPOCHS', 0)
        min_lr = getattr(cfg.SOLVER, 'MIN_LR', 0.0)
        max_epochs = cfg.SOLVER.EPOCHS
        
        if epoch < warmup_epochs:
            # Warmup phase, linear increase learning rate
            warmup_factor = getattr(cfg.SOLVER, 'WARMUP_FACTOR', 0.1)
            alpha = epoch / warmup_epochs
            warmup_lr = cfg.SOLVER.LR * warmup_factor
            new_lr = warmup_lr + (cfg.SOLVER.LR - warmup_lr) * alpha
        else:
            # Cosine annealing phase
            progress = (epoch - warmup_epochs) / (max_epochs - warmup_epochs)
            new_lr = min_lr + 0.5 * (cfg.SOLVER.LR - min_lr) * (1 + np.cos(np.pi * progress))
        
        for param_group in optimizer.param_groups:
            param_group["lr"] = new_lr
        return new_lr
    else:
        # Original step-wise learning rate decay strategy
